...
- 요청 처리 과정
    1. 클라이언트가 서비스의 IP 주소와 포트로 요청을 보냄
    1. Kube Proxy는 요청을 어떤 Pod로 전달해야 하는지 결정하기 위해 Kubernetes의 서비스 검색 기능을 사용
    1. Kube Proxy는 Pod의 IP 주소와 포트 정보를 사용하여 요청을 적절한 Pod로 포워딩
    1. Kube Proxy는 Pod의 응답을 클라이언트에게 전달
